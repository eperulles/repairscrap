...
class PCBAScreen(MDScreen):
    ...
    def on_enter(self):
        self.load_data()
        self.setup_ui() # Refrescar UI para ver el modo actual (Reparación o Scrap)
    ...
    def focus_next_field_in_list(self):
        self.dismiss_menu()
        fields = [self.qr_field, self.modelo_field, self.area_field, self.linea_field, self.supervisor_field, self.defecto_field, self.reparacion_field, self.ubicacion_field]
    ...
    def setup_ui(self):
        ...
        current_mode = app.movimiento_tipo # "Reparación" o "Scrap"
        ...
        header_text = f"REGISTRO PCBA ({current_mode.upper()})"
        ...
        form_card = MDCard(orientation="vertical", padding="24dp", spacing="12dp", size_hint_y=None, radius=[20, 20, 20, 20], md_bg_color=card_bg, elevation=2)
        form_card.bind(minimum_height=form_card.setter('height'))
        
        fill = (0.04, 0.08, 0.22, 1) if is_dark else (1, 1, 1, 1)
        self.qr_field = self.create_field("Escanear Código QR", "qrcode-scan", fill)
        form_card.add_widget(self.qr_field)
    ...
    def submit_form(self, *a):
        qr = self.qr_field.text.strip(); from kivymd.app import MDApp; app = MDApp.get_running_app()
        if not qr: self.status_label.text = "QR Code requerido"; return
